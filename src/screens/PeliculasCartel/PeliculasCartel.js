...
function Cartel(props){
    ...
    function filtrar(){
        setDatosCopia(
            datosC.filter(movie => movie.original_title.toLowerCase().includes(filtro.toLowerCase()))
        );
    }

    function cargarMas(){
        fetch (`https://api.themoviedb.org/3/movie/now_playing?api_key=72a023a2665eca4e7abeca593a5c2e2a&page=${pagina}`)
        .then(response => response.json())
        .then(data => {
            setDatosC(datosC.concat(data.results));
    ...
    return(
        <div>
            ...
            <button className="btn alert-primary" onClick={() => cargarMas()}>
                Cargar más
            </button>
        </div>
    )
